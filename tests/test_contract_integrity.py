...
from invoices import paths, ingest, tokenize, candidates, emit, utils, labels, train
    ...
    def test_contract_schema_completeness(self):
        ...
        for _, doc_info in indexed_docs.iterrows():
            ...
            for version_field in version_stamps:
                ...
                assert value is not None, f"Version stamp {version_field} should not be null in {doc_id}"
                assert isinstance(value, str), f"Version stamp {version_field} should be string in {doc_id}"
                assert len(value) > 0, f"Version stamp {version_field} should not be empty in {doc_id}"
            ...
            assert isinstance(predictions['extensions'], dict), f"Extensions should be dict in {doc_id}"
            assert isinstance(predictions['experimental'], dict), f"Experimental should be dict in {doc_id}"
            ...
            for schema_field in schema_fields:
                ...
                if status != 'PREDICTED':
                    assert value is None, f"Value should be null for non-PREDICTED field {schema_field} in {doc_id}"
