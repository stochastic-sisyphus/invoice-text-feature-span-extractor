...
from invoices import paths, ingest, tokenize, candidates
...
class TestCandidateBounds:
    ...
    def test_candidate_count_bounds(self):
        ...
        for _, doc_info in indexed_docs.iterrows():
            ...
            # Check bound
            assert candidate_count <= 200, f"Document {doc_id} has {candidate_count} candidates (max 200)"
            
            # Load candidates to verify
            candidates_df = candidates.get_document_candidates(sha256)
            assert len(candidates_df) == candidate_count, "Candidate count should match DataFrame length"
            assert len(candidates_df) <= 200, "Candidates DataFrame should not exceed 200 rows"
        
        print(f"✓ Candidate bounds test passed: all documents ≤ 200 candidates")
    ...
    def test_bucket_distribution(self):
        ...
        expected_buckets = {
            'date_like', 'amount_like', 'id_like', 'keyword_proximal', 'random_negative'
        }
        ...
        for _, doc_info in indexed_docs.iterrows():
            ...
            if len(tokens_df) >= 10:  # Only check distribution for docs with enough tokens
                # Should have at least 3 different bucket types
                assert len(found_buckets) >= 3, f"Document {doc_id} should have at least 3 bucket types"
                
                # Check that buckets make sense
                for bucket in found_buckets:
                    assert bucket in expected_buckets, f"Unknown bucket type: {bucket}"
                    assert bucket_counts[bucket] > 0, f"Bucket {bucket} should have positive count"
    ...
    def test_candidate_features(self):
        ...
        required_features = [
            # Basic info
            'candidate_id', 'doc_id', 'sha256', 'page_idx', 'token_idx', 'token_id', 'text', 'bucket',
            # Bounding box
            'bbox_norm_x0', 'bbox_norm_y0', 'bbox_norm_x1', 'bbox_norm_y1',
            # Text features
            'text_length', 'digit_ratio', 'uppercase_ratio', 'currency_flag', 'unigram_hash', 'bigram_hash',
            # Geometry features
            'center_x', 'center_y', 'width_norm', 'height_norm', 'distance_to_top', 'distance_to_bottom',
            'distance_to_left', 'distance_to_right', 'distance_to_center',
            # Style features  
            'font_size', 'font_size_z', 'is_bold', 'is_italic', 'font_hash',
            # Context features
            'context_bow_hash', 'same_row_count', 'block_index', 'local_density', 'is_remittance_band'
        ]
        ...
        for _, doc_info in indexed_docs.iterrows():
            ...
            for feature in required_features:
                assert feature in candidates_df.columns, f"Required feature {feature} missing from candidates for {doc_id}"
            
            # Check data types and ranges
            assert candidates_df['page_idx'].min() >= 0, "Page indices should be non-negative"
            assert candidates_df['token_idx'].min() >= 0, "Token indices should be non-negative"
            assert candidates_df['text_length'].min() > 0, "Text length should be positive"
            assert candidates_df['digit_ratio'].min() >= 0, "Digit ratio should be non-negative"
            assert candidates_df['digit_ratio'].max() <= 1, "Digit ratio should be ≤ 1"
            assert candidates_df['uppercase_ratio'].min() >= 0, "Uppercase ratio should be non-negative"
            assert candidates_df['uppercase_ratio'].max() <= 1, "Uppercase ratio should be ≤ 1"
            ...
            assert (candidates_df['bbox_norm_x1'] >= candidates_df['bbox_norm_x0']).all(), "x1 should be >= x0"
            assert (candidates_df['bbox_norm_y1'] >= candidates_df['bbox_norm_y0']).all(), "y1 should be >= y0"
    ...
    def test_nms_functionality(self):
        ...
        for _, doc_info in indexed_docs.iterrows():
            ...
            assert overlap_ratio < 0.1, f"Too many overlapping candidates remain after NMS: {high_overlap_count}/{len(candidates_df)}"
