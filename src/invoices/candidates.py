...
INVOICE_KEYWORDS = {
    ...
    'due date', 'payment due', 'date due', 'total', 'amount',
    'balance', 'statement', 'document', 'bill', 'billing'
}
...
class SpanBuilder:
    ...
    def __init__(self, max_gap: float = 0.02):
...
class ProximityScorer:
    ...
    def compute_proximity_score(self, bbox_norm: Tuple[float, float, float, float]) -> float:
        ...
        for anchor_x, anchor_y, _ in self.cue_anchors:
            ...
            distance = math.sqrt(dx*dx + dy*dy) - same_line_bonus - reading_order_bonus
...
def compute_text_features_enhanced(text: str) -> Dict[str, Any]:
    ...
    if not text_clean:
        return {
            'text_length': 0, 'digit_ratio': 0.0, 'uppercase_ratio': 0.0,
            'currency_flag': False, 'unigram_hash': '', 'bigram_hash': ''
        }
...
def compute_geometry_features_enhanced(bbox_norm: Tuple[float, float, float, float], 
...
def compute_style_features_enhanced(font_size: float, is_bold: bool, is_italic: bool, 
                                  font_hash: str, page_font_sizes: List[float]) -> Dict[str, Any]:
...
def compute_local_density_grid(bbox_norm: Tuple[float, float, float, float], 
                              page_grid: PageGrid, window_size: float = 0.1) -> float:
    ...
    for neighbor in neighbors:
        ...
        if isinstance(neighbor, dict) and 'center_x' in neighbor and 'center_y' in neighbor:
...
def compute_section_prior(bbox_norm: Tuple[float, float, float, float], page_idx: int) -> float:
...
def apply_soft_nms_grid(candidates: List[Dict[str, Any]], page_grid: PageGrid, 
    ...
    candidates_sorted = sorted(candidates, key=lambda x: x.get('total_score', 0.0), reverse=True)
...
def diversity_sampling(candidates: List[Dict[str, Any]], max_candidates: int = 200) -> List[Dict[str, Any]]:
...
def generate_candidates_enhanced(sha256: str) -> Tuple[int, Dict[str, Any]]:
    ...
    for page_idx in sorted(tokens_df['page_idx'].unique()):
        ...
        for span in spans:
            ...
            style_features = compute_style_features_enhanced(
                span['font_size'], span['is_bold'], span['is_italic'], 
                span['font_hash'], page_font_sizes
            )
        ...
        print(f"  Page {page_idx}: {len(page_candidates)} candidates in {page_time:.3f}s")
    ...
    if all_candidates:
        ...
        print(f"  Coverage: {coverage_stats['cue_proximal_spans']}/{coverage_stats['total_spans']} cue-proximal, "
        ...
        print(f"  Timing: spans={timings['spans']:.3f}s, scoring={timings['scoring']:.3f}s, "
